...
/* the following lookup tables are for j=0..4 */
int klimit[]={1,2,4,8,0x10};              /* 1<<j     */
int llimit[]={0x10,8,4,2,1};              /* 1<<(4-j) */
int modval[]={0x1ff,0xff,0x7f,0x3f,0x1f}; /* 1<<(9-j) */
	...
	/* out */ Byte simoutput[12])
{
	...
	for (i=1; i<9; i++) {
		...
		for (j=0; j<5; j++)
			for (k=0; k<klimit[j]; k++)
				for (l=0; l<llimit[j]; l++) {
					m = l + (k<<(5-j));
					n = m + llimit[j];
					y = (x[m]+(x[n]<<1)) & modval[j];
					z = ((x[m]<<1)+x[n]) & modval[j];
					x[m] = table[j][y];
					x[n] = table[j][z];
				}
...
/*
** do two rounds of COMP128
** This function is used once we have a collision.
*/
void A3A8_tworounds(/* in */ Byte rand[16], /* in */ Byte key[16],
	/* out */ Byte x[32])
{
	Byte bit[128];
	int i, j, k, l, m, n, y, z, next_bit;
