...
@ISA = qw(Exporter);
@EXPORT = qw(open_port open_listen open_proc);
@EXPORT_OK = qw(print close select);
...
## $handle = Chat2->open_port("server.address",$port_number);
...
## ($host, $port, $handle) = Chat2->open_listen([$port_number]);
...
sub open_listen { ## public
	...
	*S = ++$next;
	...
	(@myaddr, $port, bless(\*S)); # returning this
}

## $handle = Chat2->open_proc("command","arg1","arg2",...);
...
sub open_proc { ## public
	...
	*S = ++$next;
	...
	croak "Cannot find a new pty" unless defined $pty;
	$pid = fork;
	...
	unless ($pid) {
		...
		open(STDOUT,">&TTY");
		open(STDERR,">&STDOUT");
		croak "Oops" unless fileno(STDERR) == 2;	# sanity
		close(S);
		exec @cmd;
		croak "Cannot exec @cmd: $!";
	}
	close(TTY);
	bless \*S;
}
...
## $return = $handle->expect($timeout_time,
## 	$pat1, $body1, $pat2, $body2, ... )
## $handle is from previous Chat2->open_*().
...
## $handle->print(@data)
## $handle is from previous Chat2->open().
...
## @ready_handles = Chat2->select($timeout, @handles)
